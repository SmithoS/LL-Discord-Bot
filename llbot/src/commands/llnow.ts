import { LLNowClient } from "../lib/LLNowClient";
...
module.exports = {
  ...
  descriptionExAry: [
    `　オプション "${OPTION_START01}"または"${OPTION_START02}" ：\n　　定期的にツイートを表示します。`,
  ...
  callback: async ({ channel, args, interaction }) => {
    ...
    switch (option) {
      ...
      case OPTION_START02:
        ...
        const currentIntervalId = MyCache.get(
          MyConst.CACHE_KEY_LLNOW_INTERVAL_KEY
        );
        if (currentIntervalId != null) {
          clearAutoGetTweet();
          interaction.reply(
            `LL Now の最新ツイート取得処理を再設定します。${MyConst.LLNOW_AUTO_SEND_INTERVAL_MINUTES}分ごとに自動取得します。`
          );
        } else {
          interaction.reply(
            `LL Now の最新ツイートを${MyConst.LLNOW_AUTO_SEND_INTERVAL_MINUTES}分ごとに自動取得します。`
          );
        }
        ...
        const sendFunction = async () => {
          try {
            const replyText: string = await getOneTweetAndReplyText();
        ...
        const intervalId = setInterval(
          sendFunction,
          MyConst.LLNOW_AUTO_SEND_INTERVAL_MINUTES * 60 * 1000
        );
        ...
        const timeoutID = setTimeout(() => {
          ...
          if (interId != null) {
            ...
            channel.send(
              "LL Now のツイート自動取得を長時間実行しているため、強制停止します。"
            );
          }
      ...
      case OPTION_GET02:
        ...
        try {
          ...
          replyText =
            "ツイート取得処理でエラーが発生しました。時間をおいて再度試してみてください。";
        }
...
async function getOneTweetAndReplyText() {
  ...
  const tweetList: Tweet[] = await LLNowClient.getRecentTweets(1);
  if (tweetList.length > 0) {
    tweet = tweetList[0];
  }

  // 以前の取得ツイートを取得
  const beforeTweet: Tweet = MyCache.get(
    MyConst.CACHE_KEY_LLNOW_CURRENT_REPLY_TWEET
  );
