...
const LLFansClient = require('../lib/LLFansClient');


module.exports = {
    category: 'llfans commands',
    description: '現在停止中（LL-Fansから楽曲情報を取得する予定のコマンドです。LL-Fansの調整中につき一時停止中。）',
//    descriptionExAry: ['パラメータに曲名または曲名の一部を指定してください。'],
    ...
    callback: async ({ args, channel, interaction }) => {
        ...
        await interaction.editReply({
            content: '申し訳ありませんが、負荷軽減用のAPI作成中につき動作を一時停止中です。'
        });
...
async function replySong(songId, interaction) {
    ...
    if (lastSetlist == null) {
        ...
        performanceStageDisplayName += (lastSetlist.getConcertName() == '-' ? '' : `【${ lastSetlist.getConcertName() }】`);
        performanceStageDisplayName += (lastSetlist.getName() == '-' ? '' : `（${ lastSetlist.getName() }）`);
        embed.setFields([
            {
                name: '最新の披露と日付',
