...
class LLNowClient {
    ...
    static async getRecentTweets(count) {
        ...
        if (beforeCallMoment != null) {
            if (MyMoment.getJSTnow().diff(beforeCallMoment, 'seconds') < MyConst.LLNOW_CALL_API_COOLDOWN_MINUTES * 60) {
                return [];
            }
        }
        ...
        // API呼び出し時刻を更新
        MyCache.set(MyConst.CACHE_KEY_LLNOW_BEFORE_CALL_MOMENT, MyMoment.getJSTnow());
